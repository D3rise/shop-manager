...
class MoneyRequestsModule extends BaseModule {
  ...
  newMoneyRequest(amountInEther) {
    const amountInWei = this.web3.web3.utils.toWei(
      String(amountInEther),
      "ether"
    );
    return this.contract.methods
      .newMoneyRequest(amountInWei)
  ...
  async __operateMoneyRequest(city, accept) {
    ...
    return this.contract.methods
      .approveMoneyRequest(city, accept)
      .send({ from: this.web3.userAddress, value: request.count });
  }
}
