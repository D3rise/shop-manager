...
class ReviewsModule extends BaseModule {
  ...
  async getShopReviewIds(city) {
    const { exists, reviewIds } = await this.contract.methods.getShopReviews(city).call();
    return [exists, reviewIds]
  }

  async getUserReviewIds(userAddress) {
    const { exists, reviewIds } = await this.contract.methods.getUserReviews(userAddress).call();
    return [exists, reviewIds]
  }
}
